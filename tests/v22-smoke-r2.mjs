...
const addFailure = (scope, message, details = {}) => failures.push({ scope, message, ...details });

async function openPage(page, pathname, scope) {
  const response = await page.goto(`${base}${pathname}${pathname.includes('?') ? '&' : '?'}v22-smoke=${Date.now()}`, {
    waitUntil: 'networkidle2',
    timeout: 90_000,
  });
  if (!response || response.status() >= 400) {
    addFailure(scope, 'Page request failed', { status: response?.status(), url: page.url() });
    return false;
  }
  return true;
}
...
async function inspectIndex(viewport) {
  ...
  page.on('requestfailed', (request) => {
    ...
    if (!/google-analytics|googletagmanager|doubleclick|favicon|chrome-extension/i.test(url)) {
      requestFailures.push({ url, error: request.failure()?.errorText || 'unknown' });
    }
  });

  if (!(await openPage(page, '/', viewport.name))) {
    await page.close();
    return;
  }
  ...
  if (before.marker !== build) addFailure(viewport.name, 'Missing V22 build marker', before);
  if (before.obsoleteVisible) addFailure(viewport.name, 'Obsolete site title remains visible');
  if (before.scrollWidth > before.clientWidth + 6) addFailure(viewport.name, 'Unexpected page-level horizontal overflow', before);
  ...
  const menuResult = await page.evaluate(() => {
    ...
    const hadTrigger = checkbox instanceof HTMLInputElement || clickTarget instanceof HTMLElement;
  ...
  if (menuResult.hadTrigger && !menuResult.found) addFailure(viewport.name, 'Menu trigger exists but no panel can be resolved', menuResult);
  if (menuResult.found) {
    if (menuResult.width >= menuResult.viewportWidth * 0.96) addFailure(viewport.name, 'Menu still spans the viewport', menuResult);
    if (menuResult.height >= menuResult.viewportHeight * 0.98) addFailure(viewport.name, 'Menu still replaces the full page', menuResult);
    if (menuResult.bodyOverflowY === 'hidden') addFailure(viewport.name, 'Menu locks body scrolling', menuResult);
    if (!menuResult.mainVisible) addFailure(viewport.name, 'Opening menu hides page content', menuResult);
  }

  if (requestFailures.length) addFailure(viewport.name, 'Static request failures', { requestFailures });
  const relevantConsoleErrors = consoleErrors.filter((text) => !/favicon|ResizeObserver loop|analytics|third.party/i.test(text));
  if (relevantConsoleErrors.length) addFailure(viewport.name, 'Console errors', { consoleErrors: relevantConsoleErrors });
...
async function inspectData() {
  ...
  if (!(await openPage(page, '/', 'authority-map-host'))) {
    await page.close();
    return;
  }
  ...
  if (payload.status !== 200) addFailure('authority-map', 'Authority map request failed', { status: payload.status });
  if (entries.length < 100) addFailure('authority-map', 'Authority map is unexpectedly small', { entries: entries.length });
  const kanaTranslations = entries.filter((entry) => /[ぁ-ゖァ-ヺー]/u.test(entry.zh || ''));
  if (kanaTranslations.length) addFailure('authority-map', 'Chinese authority values still contain kana', { count: kanaTranslations.length, sample: kanaTranslations.slice(0, 20) });
  ...
  for (const [prefix, expected] of officialSamples) {
    ...
    if (entry && !String(entry.zh || '').includes(expected)) {
      addFailure('authority-map', `Official memory title mismatch for ${prefix}`, { entry, expected });
    }
  }
  results.push({ scope: 'authority-map', status: payload.status, entries: entries.length, kanaTranslations: kanaTranslations.length });
  await page.close();
}
...
async function inspectEditor() {
  ...
  if (state.scrollWidth > state.clientWidth + 6) addFailure('story-title-editor', 'Mobile editor overflows the viewport', state);
  if (state.numericRows >= 10 && state.numericInversions > 0) addFailure('story-title-editor', 'No.n rows are not naturally sorted', state);
